...
const createBookingData = (data) => ({
  ...
  date: data.date,
  time: data.time,
  duration: data.duration || 1, // 1 or 2 hours
...
export const calculatePricing = (basePricePerHour, duration, isFirstBooking = false) => {
  // Remove duration-based discounts - charge full price for each hour
...
export const useBookingStore = create((set, get) => ({
  ...
  getContiguousSlots: (selectedTime) => {
    ...
    const maxHours = Math.min(2, 21 - timeSlot); // Max 2 hours or until 9 PM
