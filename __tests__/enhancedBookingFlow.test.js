...
describe('Enhanced Booking Flow Tests', () => {
  ...
  describe('Pricing Calculation with First-Time Discount', () => {
    ...
    test('should calculate correct pricing for 2 hour booking with first-time discount only', () => {
      ...
      const expectedSubtotal = 15000 * 2; // No duration multiplier for 2 hours
      const expectedFirstTimeDiscount = expectedSubtotal * 0.1;
      const expectedDurationDiscount = 0; // No duration discount
  ...
  describe('Time Slot Validation', () => {
    ...
    test('should validate time slot availability within operating hours', () => {
      ...
      expect(validateTimeSlot(19, 2)).toBe(true); // Would not exceed 9 PM
    });
    
    test('should generate correct contiguous slot options', () => {
      const getContiguousSlots = (startHour) => {
        const maxDuration = Math.min(2, 21 - startHour); // Max 2 hours now
        const slots = [];
      ...
      expect(getContiguousSlots(9)).toEqual([1, 2]); // Early slot, all durations available
